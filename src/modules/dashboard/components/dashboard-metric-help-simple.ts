...
export const DASHBOARD_HELP_SIMPLE_WORDS: Record<string, DashboardMetricSimpleWords> = {
  ...
  kpi_active_subscriptions: S({
    paragraphs: [
      "Suscripción = algo que te cobra seguido (Netflix, gimnasio, etc.). “Activas” son las que no pausaste ni cancelaste.",
      "El costo mensual aproximado es una suma de lo que la app calcula según el monto y cada cuánto paga cada una.",
    ],
  }),

  kpi_upcoming_payments: S({
    paragraphs: [
      "Acá la app intenta decir: “en los próximos días, ¿cuánta plata vas a tener que pagar o cobrar según lo que ya cargaste?”",
      "Suma montos de cosas con fecha cercana (cuotas, suscripciones, obligaciones) para que no te agarre de sorpresa.",
    ],
  }),

  kpi_overdue: S({
    paragraphs: [
      "“Vencido” es plata que según tus fechas ya debió pagarse o cobrarse y todavía está pendiente.",
      "Es una señal de “esto necesita atención”: registrar el pago, negociar o actualizar la fecha.",
    ],
  }),

  kpi_transferred: S({
    paragraphs: [
      "Transferencia = moviste plata de una cuenta tuya a otra cuenta tuya. No es que el mundo te regaló plata ni que la perdiste: solo cambió de cajón.",
      "Por eso no suma como ingreso “nuevo”, pero igual cuenta cuánto te movés entre cuentas.",
    ],
  }),
  ...
  adv_liquidity: S({
    paragraphs: [
      "Liquidez = plata que podés usar pronto, sin vender una casa ni tocar cosas muy trabadas.",
      "Acá suele entrar efectivo, banco y ahorros que definiste como “listos para usar”.",
    ],
  }),
  ...
  adv_projected_cash_30: S({
    paragraphs: [
      "La app mira pagos y cobros que ya conoce para los próximos ~30 días y hace una cuenta aproximada de cómo podría quedar tu saldo.",
      "Es una adivinanza educada: si falta cargar algo, el número se va a equivocar.",
    ],
  }),
  ...
  shared_principal: S({
    paragraphs: [
      "“Principal” acá es el monto base de esos registros compartidos: el tamaño de lo que estás mirando en solo lectura.",
    ],
  }),

  shared_pending: S({
    paragraphs: [
      "“Pendiente” = todavía no está todo pagado o cobrado en esos registros que alguien compartió con vos.",
    ],
  }),
  ...
  widget_upcoming_recent: S({
    paragraphs: [
      "Un lado: lo que se viene (pagos/cobros próximos). Otro lado: movimientos recientes que ya pasaron.",
    ],
  }),

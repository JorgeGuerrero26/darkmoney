/**
 * PORT de DarkMoneyApp/features/dashboard/lib/dashboard-ai-content.ts.
 * Tipos, catálogo de términos y parsing del texto generado por la IA del
 * dashboard. Sin dependencias de React Native (el GEMINI_BRAND usa hex web).
 */
...
export const DASHBOARD_AI_TONE_OPTIONS: Array<{
  id: DashboardAiTone;
  label: string;
  description: string;
}> = [
...
/** Colores de marca para el badge de IA (chartIndigo/teal/coral/gold del móvil). */
...
export function ensureDashboardAiComplexTerms(
  reply: string,
  terms: DashboardAiComplexTerm[],
) {
  if (!reply) return [];
...
export function buildDashboardAiTextParts(
  reply: string,
  terms: DashboardAiComplexTerm[],
): DashboardAiTextPart[] {
  if (!reply) return [];
  const resolvedTerms = ensureDashboardAiComplexTerms(reply, terms).sort(
